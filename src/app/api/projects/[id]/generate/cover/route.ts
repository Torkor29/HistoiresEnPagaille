...
export async function POST(
  ...
  return withProjectLock(projectId, async () => {
    ...
    if (!title) {
      title = project.story?.coverTitle || project.title || 'Mon histoire';
    }
    ...
    // Priorité : CHARACTER_REF (fiches stylisées) > PHOTO_REF (photos originales)
    ...
    if (!hasCharacterRefImages && characters.length > 0) {
      ...
      if (main) {
        characterDescriptor = getCharacterDescriptionForScene(main, null);
      }
    }

    // Tenue cohérente avec l'histoire : contexte le plus fréquent dans les scènes (ou défaut)
    ...
    await prisma.asset.create({
      data: {
        ...
        type: 'COVER_IMAGE',
        url: key,
        metadata: JSON.stringify({ title: fullTitle }),
      },
    });
